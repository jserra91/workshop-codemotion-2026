...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (stores?.solicitudes?.$solicitudesCount) {
      setSolicitudesCount(stores.solicitudes.$solicitudesCount.get());
      const unsub = stores.solicitudes.$solicitudesCount.subscribe((v) => setSolicitudesCount(v));
      return unsub;
    }
  }, []);
      ...
      <div className="balance-card">
        ...
        {solicitudesCount > 0 && (
          <div style={{ marginTop: 8, fontSize: 12, opacity: 0.9 }}>
            📋 {solicitudesCount} solicitud(es) pendiente(s)
          </div>
        )}
      </div>
