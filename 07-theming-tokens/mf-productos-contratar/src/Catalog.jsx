...
export default function Catalog() {
  const handleContratar = (producto) => {
    window.__MF_STORES__.solicitudes.addSolicitud(producto);
    window.dispatchEvent(
      new CustomEvent('mf:notification', {
        detail: {
          type: 'success',
          message: `Solicitud de ${producto.nombre} enviada`,
        },
      })
    );
  };
      ...
      <div className="catalogo-list">
        {catalogo.map((p, i) => (
          <div className={`catalogo-card ${p.destacado ? 'destacado' : ''}`} key={i}>
            ...
            <button className="btn-contratar" onClick={() => handleContratar(p)}>Contratar</button>
          </div>
        ))}
      </div>
    </div>
