import React, { useState, useEffect } from "react";
import "./styles.css";

// ── Nombre del evento: mismo contrato que en mf-productos-contratar ──
const EVENTO_CARRITO = "mf:carrito-actualizado";

export default function Header() {
  // Lista de productos seleccionados recibida desde otro microfrontend
  const [productos, setProductos] = useState([]);

  useEffect(() => {
    // Escuchar el Custom Event emitido por mf-productos-contratar
    const handleCarritoActualizado = (event) => {
      setProductos(event.detail.productos);
    };

    window.addEventListener(EVENTO_CARRITO, handleCarritoActualizado);

    // Limpiar el listener cuando el microfrontend se desmonte
    return () => {
      window.removeEventListener(EVENTO_CARRITO, handleCarritoActualizado);
    };
  }, []);

  return (
    <header className="app-header">
      <div className="header-avatar">JS</div>
      <div className="header-info">
        <div className="header-greeting">Buenos días</div>
        <div className="header-name">Jordi Serra</div>
      </div>
      <div className="header-actions">
        {/* Botón de carrito: solo se muestra el badge cuando hay productos */}
        <button className="header-btn header-btn--carrito" aria-label="Productos seleccionados">
          <svg viewBox="0 0 24 24">
            <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
          </svg>
          {productos.length > 0 && (
            <span className="header-badge">{productos.length}</span>
          )}
        </button>
        <button className="header-btn" aria-label="Notificaciones">
          <svg viewBox="0 0 24 24">
            <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" />
            <path d="M13.73 21a2 2 0 0 1-3.46 0" />
          </svg>
        </button>
        <button className="header-btn" aria-label="Configuración">
          <svg viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="3" />
            <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" />
          </svg>
        </button>
      </div>
    </header>
  );
}
